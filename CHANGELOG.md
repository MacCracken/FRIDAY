...
- **Go edge retired** — Deleted `cmd/secureyeoman-edge/` (legacy Go `secureyeoman-edge` binary). CI `test-edge` now builds and smoke-tests the Rust `sy-edge` bin target of `sy-core`. Release pipeline drops `setup-go` and ships edge as `secureyeoman-<version>-edge-linux-x64` (Rust, native). Cross-compile to arm64/armv7/riscv64 deferred to a follow-up (needs `cross` or rustup target linkers).

- **Node dependency audit** — Down from 35 vulnerabilities (15 moderate / 17 high / 3 critical) to **3 moderate, 0 high, 0 critical**. Bumped direct: `@anthropic-ai/sdk` 0.80→0.90 (patches memory-tool path-traversal CVE), `dompurify` 3.3.3→3.4.0. Added/tightened transitive overrides: `protobufjs` 8.0.1 (fixes critical baileys/libsignal RCE chain), `serialize-javascript` 7.0.5, `undici` 6.25.0, `nanoid` 5.1.9, `lodash-es` 4.18.1, `zod` 3.25.76. The residual three moderates all trace to a single upstream mermaid XSS in sequence-diagram labels, reachable only through `@excalidraw/mermaid-to-excalidraw`, which hard-pins `mermaid: 10.9.3` — waiting on an upstream bump to 10.9.5+.

- **zod override pinned to v3** — `npm audit fix` bumped `openai`, `@modelcontextprotocol/sdk`, `eslint-plugin-react-hooks`, and the new `@anthropic-ai/sdk` 0.90 into requiring `zod@^4`, which npm hoisted as zod 4.3.6 at the root. Our workspaces pin `zod: ^3.23.8` but the hoisted v4 was what `tsc` imported via the type graph. zod 4's inference is expensive enough to OOM TypeScript even at 18 GB heaps. Pinning `zod: 3.25.76` in root `overrides` forces a single zod 3 across the tree (all consumers' zod peers accept both v3 and v4) and restores typecheck at the original 8 GB heap.
